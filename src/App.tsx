...
function DeliveryMap({ orders }: { orders: Order[] }) { const element = useRef<HTMLDivElement>(null); const map = useRef<L.Map | null>(null); useEffect(() => { if (!element.current) return; const points = orders.map((order) => ({ order, coordinates: mapCoordinates(order.locationUrl) })).filter((item): item is { order: Order; coordinates: Coordinates } => Boolean(item.coordinates)); if (!map.current) { map.current = L.map(element.current, { zoomControl: false }).setView([35.7595, -5.834], 12); L.control.zoom({ position: 'bottomright' }).addTo(map.current); L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '© OpenStreetMap contributors' }).addTo(map.current) } const layer = L.layerGroup().addTo(map.current); points.forEach(({ order, coordinates }) => { L.circleMarker([coordinates.latitude, coordinates.longitude], { radius: 9, color: '#143b32', weight: 2, fillColor: order.status === 'Out for delivery' ? '#e8b643' : '#62a985', fillOpacity: 1 }).bindPopup(`<strong>${order.client}</strong><br>${order.address}`).addTo(layer) }); if (points.length) map.current.fitBounds(L.latLngBounds(points.map(({ coordinates }) => [coordinates.latitude, coordinates.longitude])), { padding: [30, 30], maxZoom: 14 }); return () => { layer.remove() } }, [orders]); return <><div ref={element} className="delivery-map" />{!orders.some((order) => mapCoordinates(order.locationUrl)) && <p className="map-empty">Add Google Maps location links to orders to see them here.</p>}</> }
