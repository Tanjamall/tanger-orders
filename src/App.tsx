...
function OrderApp({ session }: { session: Session | null }) {
  ...
  async function updateOrder(form: HTMLFormElement) {
    ...
    const values = new FormData(form)
    const updated: Order = { ...editingOrder, client: String(values.get('client')), phone: String(values.get('phone')), address: String(values.get('address')), locationUrl: String(values.get('locationUrl') || ''), assignedTo: String(values.get('assignedTo')), status: values.get('status') as Status, paymentStatus: values.get('paymentStatus') as PaymentStatus, deliveryCharge: Number(values.get('deliveryCharge')) || 0, otherExpense: Number(values.get('otherExpense')) || 0, notes: String(values.get('notes') || '') }
    setOrders((all) => all.map((order) => order.id === updated.id ? updated : order))
    if (supabase && workspaceId) { const { error } = await supabase.from('orders').update({ client_name: updated.client, phone: updated.phone, address: updated.address, location_url: updated.locationUrl || null, assigned_to: updated.assignedTo || null, status: updated.status, payment_status: updated.paymentStatus, delivery_charge: updated.deliveryCharge, other_expense: updated.otherExpense, notes: updated.notes }).eq('id', updated.id); if (error) setNotice(error.message) }
...
function OrderCard({ order, products, members, onStatus, onEdit }: { order: Order; products: Product[]; members: { id: string; display_name: string | null }[]; onStatus: (id: string, status: Status) => void; onEdit: (order: Order) => void }) {
  ...
  return <article className="order-card compact-order"><div className="compact-order-head"><div className="client-block"><h3>{order.client}</h3><a href={`https://wa.me/${order.phone.replace(/\D/g, '')}`} target="_blank">{order.phone} ↗</a></div><div className="order-head-actions"><button className="edit-order" title="Edit order" onClick={() => onEdit(order)}>✎</button><select className={`status-picker ${order.status.toLowerCase().replaceAll(' ', '-')}`} aria-label="Order status" value={order.status} onChange={(event) => void onStatus(order.id, event.target.value as Status)}>{statuses.map(status => <option key={status}>{status}</option>)}</select></div></div><p className="compact-items">{lines}</p><p className="compact-address">⌖ {order.address}</p><div className="compact-meta"><span>◉ {assignee}</span><span>{order.paymentStatus}</span><b>{money(total)}</b></div></article>
}

function NavButton({ icon, label, active, onClick }: { icon: 'orders' | 'inventory' | 'profit'; label: string; active: boolean; onClick: () => void }) { return <button className={active ? 'nav-active' : ''} onClick={onClick}><NavIcon name={icon} />{label}</button> }
function NavIcon({ name }: { name: 'orders' | 'inventory' | 'profit' }) { const common = { fill: 'none', stroke: 'currentColor', strokeWidth: 1.9, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const }; return <svg viewBox="0 0 24 24" aria-hidden="true">{name === 'orders' && <><rect x="4" y="3.5" width="16" height="17" rx="2.5" {...common} /><path d="M8 8h8M8 12h8M8 16h5" {...common} /></>}{name === 'inventory' && <><path d="M4 8.5 12 4l8 4.5v8L12 21l-8-4.5z" {...common} /><path d="M4 8.5 12 13l8-4.5M12 13v8" {...common} /></>}{name === 'profit' && <><path d="M4 19.5V13m5 6.5V9m5 10.5V5m5 14.5v-8" {...common} /><path d="m4 9 5-3 5 2 6-4" {...common} /></>}</svg> }
