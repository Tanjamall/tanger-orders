...
function OrderCard({ order, products, members, onStatus }: { order: Order; products: Product[]; members: { id: string; display_name: string | null }[]; onStatus: (id: string, status: Status) => void }) {
  ...
  const assignee = members.find(member => member.id === order.assignedTo)?.display_name || order.assignedTo
  return <article className="order-card"><div className="order-main"><div className="order-title"><div><h3>{order.client}</h3><a href={`https://wa.me/${order.phone.replace(/\D/g, '')}`} target="_blank">{order.phone} ↗</a></div><span className={`status ${order.status.toLowerCase().replaceAll(' ', '-')}`}>{order.status}</span></div><p className="items">{lines}</p><p className="address">⌖ {order.address}</p><div className="order-meta"><span>◉ {assignee}</span><span>{order.paymentStatus}</span><b>{money(order.items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0) + order.deliveryCharge)}</b></div></div><select aria-label="Order status" value={order.status} onChange={(event) => void onStatus(order.id, event.target.value as Status)}>{statuses.map(status => <option key={status}>{status}</option>)}</select></article>
}
